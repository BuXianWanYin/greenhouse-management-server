 ...
 */
@Component
public class AgricultureWaterQualityRecent10DaysImportTask {

    private static final Logger log = LoggerFactory.getLogger(AgricultureWaterQualityRecent10DaysImportTask.class);
    ...
    private void importDailyStatsFor10Days(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        while (!currentDate.isAfter(endDate)) {
            try {
                ...
                log.info("处理水质日统计日期：{}", currentDate);
                ...
                log.error("处理水质日统计日期 {} 时发生错误：{}", currentDate, e.getMessage());
                currentDate = currentDate.plusDays(1);
            }
        }

        log.info("水质日统计数据导入完成，共处理 {} 天的数据", processedDays);
    }
    ...
    private void importHourlyStatsFor10Days(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        while (!currentDateTime.isAfter(endDateTime)) {
            try {
                ...
                log.info("处理水质小时统计时间：{}", currentDateTime);
                ...
                if (processedHours % 24 == 0) {
                    log.info("已处理 {} 小时的水质数据", processedHours);
                }

            } catch (Exception e) {
                log.error("处理水质小时统计时间 {} 时发生错误：{}", currentDateTime, e.getMessage());
                currentDateTime = currentDateTime.plusHours(1);
            }
        }

        log.info("水质小时统计数据导入完成，共处理 {} 小时的数据", processedHours);
    }
